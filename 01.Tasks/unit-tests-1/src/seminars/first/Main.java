...
public class Main {
    public static void main(String[] args) {
        ...
        System.out.println(sum(0, 2_147_483_647));
    ...
    public static int sum(int a, int b) {
        ...
        assert Math.abs(a + b) > 0 :  "Переполнение буфера";
        return a + b;
    }
    ...
    public static void happyNY() throws ParseException {
        Calendar calendar = Calendar.getInstance();
        DateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss");
        String currentDateTime = dateFormat.format(calendar.getTime());
        Date current = dateFormat.parse(currentDateTime);
        Date newYear = dateFormat.parse("01/01/2023 00:00:00");

        assert newYear.compareTo(current) <= 0 : "Еще 2022 год :(";
    ...
    public static void checkingHero() {
        ...
        */
        ...
        assert emmett.getArmorStrength() == 50: "Броня не 50";
        assert emmett.getWeapon() == "sword": "Нет меча";
        assert emmett.getBag().size() == 3: "Не 3 предмета";
        assert emmett.getBag().containsAll(List.of("Bow", "Axe", "Gold")): "Не содержит болта, топора или золота";
        assert emmett.isHuman() : "Не человек";
